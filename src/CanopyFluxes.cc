...
//    if (irrigated[Land.vtype] == 1.0 && elai > irrig_min_lai && btran < irrig_btran_thresh) {
...
void InitializeFlux(
  const LandType& Land,
  const double& dtime_in,
  const int& snl_in,
  const int& frac_veg_nosno_in,
  const double& frac_sno_in,
  ...
  const double h2osoi_liq[nlevgrnd+nlevsno],
  const double dz[nlevgrnd+nlevsno],
  const double rootfr[nlevgrnd],
  ...
  const double *smpso,
  const double *smpsc,
  const double& elai_in,
  const double& esai_in,
  const double& emv_in,
  const double& emg_in,
  const double& qg_in,
  const double& t_grnd_in,
  const double& forc_t,
  const double& forc_pbot_in,
  const double& forc_lwrad_in,
  const double& forc_pco2,
  const double& forc_po2,
  const double& forc_u_in,
  const double& forc_v_in,
  const double& forc_q_in,
  const double& forc_th_in,
  const double& z0mg_in,
  const double& t_veg_in,
...
// calc_root_moist_stress()
{
  ...
  if (!Land.lakpoi && !Land.urbpoi) {
    ...
    if (frac_veg_nosno == 0) {
      ...
      calc_root_moist_stress(Land.vtype, h2osoi_liqvol, rootfr, t_soisno, 
        tc_stress, sucsat, watsat, bsw, smpso, smpsc, eff_porosity, 
        altmax_indx, altmax_lastyear_indx, rootr, btran);
...
// we operate on single cells, so we will replace the filter criteria with something else
{
  ...
  if (!Land.lakpoi && !Land.urbpoi && frac_veg_nosno != 0) {
    ...
    while (itlef <= itmax && !stop) {
      ...
      snow_depth_c = 0.05; // critical depth for 100% litter burial by snow (=litter thickness) -- litter thickness hardwired as 0.05 m
      fsno_dl = snow_depth / snow_depth_c;                 // effective snow cover for (dry)plant litter
      elai_dl = 0.5 * (1.0 - std::min(fsno_dl, 1.0));   // exposed (dry)litter area index -- liter area hardwired as 0.5 m^2 m^-2
      ...
      if (zeta >= 0.0) {      //stable
        ...
        wc = pow((-grav * ustar * thvstar * 1000.0 / thv), 0.333); // convective boundary layer height hardwired as 1000.0
        um = std::sqrt(ur * ur + wc * wc);
      }
      obu = zldis / zeta;
      if (obuold * obu < 0.0) { nmozsgn += 1; }
      ...
      if (itlef > itmin) {
        ...
        if ((det < 0.01) && (dele < 0.1)) { stop = true; }
      }
    } // stability iteration
  } // land type
} //CanopyFluxes::StabilityIteration()
