...
// call sequence -> SurfAlbInitTimestep() -> SoilAlbedo() -> Surface
...
double calc_cosz(const double jday, const double lat, const double lon, const double declin) {
  ...
  cos(lat) * cos(declin) * cos((jday - floor(jday)) * 2.0 * ELM_PI + lon);
}
...
ArrayD1 albsod, ArrayD1 albsoi, ArrayD1 albgrd, ArrayD1 albgri, ArrayD1 albd, ArrayD1 albi, ArrayD1 fabd, 
ArrayD1 fabd_sun, ArrayD1 fabd_sha, ArrayD1 fabi, ArrayD1 fabi_sun, ArrayD1 fabi_sha, ArrayD1 ftdd, ArrayD1 ftid, 
ArrayD1 ftii, ArrayD1 flx_absdv, ArrayD1 flx_absdn, ArrayD1 flx_absiv, ArrayD1 flx_absin

void SurfAlbInitTimestep(ArrayD1 albsod, ArrayD1 albsoi, ArrayD1 albgrd, ArrayD1 albgri, ArrayD1 albd, ArrayD1 albi, ArrayD1 fabd, 
ArrayD1 fabd_sun, ArrayD1 fabd_sha, ArrayD1 fabi, ArrayD1 fabi_sun, ArrayD1 fabi_sha, ArrayD1 ftdd, ArrayD1 ftid, 
ArrayD1 ftii, ArrayD1 flx_absdv, ArrayD1 flx_absdn, ArrayD1 flx_absiv, ArrayD1 flx_absin, ArrayD2 mss_cnc_aer_in_fdb) {
...
void SurfaceAlbedo() {
  ...
  // calc with dt, not dt+1;
  // probably replace this with slope/aspect solar factor from ATS
  double coszen = calc_cosz(jday, lat, lon, declinp);
...
//  } // if !Land.urbpoi
  ...
  // Create solar-vegetated flags for the following calculations
// bool vegsol = false;
// bool novegsol = false;
// if (!Land.urbpoi) {
//   if (coszen > 0.0) {
...
//  for (int ib = 0; ib < numrad; ++ib) {
//    rho[ib] = max(rhol[Land.vtype][ib] * wl + rhos[Land.vtype][ib] * ws, mpe);
...
vegsol                     [bool] flag for (!Land.urbpoi && coszen > 0.0 && (Land.ltype == istsoil || Land.ltype == istcrop) && (elai + esai) > 0.0)
...
void TwoStream(const bool &vegsol, const int &nrad, const int &vtype, const double &coszen, const ArrayD1 xl,
...
ArrayD1 fabi_sun_z, ArrayD1 fabi_sha_z) {
  ...
  if (vegsol) {
    ...
    // Weight reflectance/transmittance by lai and sai
    double wl = elai / max(elai + esai, mpe);
    double ws = esai / max(elai + esai, mpe);
    for (int ib = 0; ib < numrad; ++ib) {
      rho[ib] = max(rhol[Land.vtype][ib] * wl + rhos[Land.vtype][ib] * ws, mpe);
      tau[ib] = max(taul[Land.vtype][ib] * wl + taus[Land.vtype][ib] * ws, mpe);
    }
    ...
    double cosz = std::max(0.001, coszen);
    double chil = std::min(std::max(xl[vtype], -0.4), 0.6);
    if (std::abs(chil) <= 0.01) {
      chil = 0.01;
    }
    double phi1 = 0.5 - 0.633 * chil - 0.330 * chil * chil;
    double phi2 = 0.877 * (1.0 - 2.0 * phi1);
    double gdir = phi1 + phi2 * cosz;
    double twostext = gdir / cosz;
    double avmu = (1.0 - phi1 / phi2 * std::log((phi1 + phi2) / phi1)) / phi2;
    double temp0 = gdir + phi2 * cosz;
    double temp1 = phi1 * cosz;
    double temp2 = (1.0 - temp1 / temp0 * std::log((temp1 + temp0) / temp1));
    ...
    for (int ib = 0; ib < numrad; ib++) {
      ...
      // Common terms
      double b = 1.0 - omega[ib] + omega[ib] * betai;
      double c1 = omega[ib] * betai;
      tmp0 = avmu * twostext;
      double d = tmp0 * omega[ib] * betad;
      double f = tmp0 * omega[ib] * (1.0 - betad);
      tmp1 = b * b - c1 * c1;
      double h = sqrt(tmp1) / avmu;
      ...
      double s1 = exp(-t1);
      double t1 = std::min(twostext * (elai + esai), 40.0);
      double s2 = exp(-t1);

      // Direct beam
      double u1 = b - c1 / albgrd[ib];
      double u2 = b - c1 * albgrd[ib];
      double u3 = f + c1 * albgrd[ib] tmp2 = u1 - avmu * h;
      ...
      if (ib == 0) {
        if (nlevcan == 1) {
          ...
          for (int iv = 0; iv < nrad; iv++) {
            ...
            if (iv == 0) {
              double laisum = 0.5 * (tlai_z[iv] + tsai_z[iv]);
            } else {
              double laisum += 0.5 * ((tlai_z[iv - 1] + tsai_z[iv - 1]) + (tlai_z[iv] + tsai_z[iv]));
            }
