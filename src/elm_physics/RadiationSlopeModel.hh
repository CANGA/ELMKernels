// stolen directly from ATS - should incorporate into ReadAtmosphere and make forc_solai and forc_solad spatially variable
// replace ELM coszen with radiation factor computed here
...
double DeclinationAngle(double doy) {
  return 23.45 * ELM_PI / 180.0 * std::cos(M_2_PI / 365 * (172 - doy));
}
...
double SolarAltitude(double delta, double phi, double tau)
{
  ...
  if (alpha <= 0.25 * ELM_PI / 180) {
    // sun is beyond the horizon
    alpha = 0.25 * ELM_PI / 180;
  }
  return alpha;
}
