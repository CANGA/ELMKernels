...
template <class ArrayDouble1D,  // a 1D array of non-const doubles, equivalent to double*
...
KOKKOS_INLINE_FUNCTION          // NATURE!
    void
    Example(const cArrayDouble1D in1, const double &in2, const cArrayInt1D in3, double &out1,
...
//
template <class ArrayDouble2D, class ArrayInt2D, class ArrayDouble1D> struct ExampleCaller {
  ...
  ExampleCaller(cArrayDouble2D in1_, cArrayDouble1D in2_, cArrayInt2D in3_, ArrayDouble1D out1_, ArrayDouble2D out2_)
      : in1(in1_), in2(in2_), in3(in3_), out1(out1_), out2(out2_) {}

  void Compute() {
    Kokkos::parallel_for(
        in1.extent(0), KOKKOS_LAMBDA(const int &g) {
          Example(Kokkos::subview(in1, i, Kokkos::ALL), in2(i),
                  Kokkos::subview(in3, i, Kokkos
                                  : ALL),
                  out1(i), Kokkos::subview(out2, i, Kokkos::ALL));
        });
  }
...
int main2() {
  ...
  // construct and initialize
  ArrayDouble2D in1(n_grid_cells, 3);
  assign(in1, 1.0);
  ArrayDouble2D out2(n_grid_cells, 3);
  assign(out2, 0.0);
  ArrayInt2D in3(n_grid_cells, 3);
  assign(in3, 1);

  ArrayDouble1D in2(n_grid_cells);
  assign(in2, 11.0);
  ArrayDouble1D out1(n_grid_cells);
  assign(in2, 11.0);
