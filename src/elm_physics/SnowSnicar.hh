...
void SNICAR_AD_RT() {
  ...
  static const int numrad_snw = 5; // number of spectral bands used in snow model [nbr]
  static const int nir_bnd_bgn = 1; // first band index in near-IR spectrum [idx]
  static const int nir_bnd_end = 4; // ending near-IR band index [idx]
  static const int sno_nbr_aer = 8; // number of aerosol species in snowpack
  static const double min_snw = 1.0e-30; // minimum snow mass required for SNICAR RT calculation [kg m-2]
  static const int ngmax = 8; // gaussian integration index
  

  // Gaussian integration angle and coefficients for diffuse radiation
  static const double difgauspt[8] // gaussian angles (radians)
  = { 0.9894009,  0.9445750,
      0.8656312,  0.7554044,
      0.6178762,  0.4580168,
      0.2816036,  0.0950125 };
  static const double difgauswt[8] //gaussian weights
  = { 0.0271525,  0.0622535,
      0.0951585,  0.1246290,
      0.1495960,  0.1691565,
      0.1826034,  0.1894506 };
  ...
  static const double c0      = 0.0;
  static const double c1      = 1.0;
  static const double c3      = 3.0;
  static const double c4      = 4.0;
  static const double c6      = 6.0;
  static const double cp01    = 0.01;
  static const double cp5     = 0.5;
  static const double cp75    = 0.75;
  static const double c1p5    = 1.5;
  static const double trmin   = 0.001;
  static const double argmax  = 10.0; // maximum argument of exponential

  // cconstant coefficients used for SZA parameterization
  static const double sza_a0 =  0.085730;
  static const double sza_a1 = -0.630883;
  static const double sza_a2 =  1.303723;
  static const double sza_b0 =  1.467291;
  static const double sza_b1 = -3.338043;
  static const double sza_b2 =  6.807489;
  static const double puny   =  1.0e-11;
  static const double mu_75  =  0.2588; // cosine of 75 degree

  // coefficients used for SZA parameterization
  double sza_c1;         // coefficient, SZA parameteirzation
  double sza_c0;         // coefficient, SZA parameterization
  double sza_factor;     // factor used to adjust NIR direct albedo
  double flx_sza_adjust; // direct NIR flux adjustment from sza_factor
  double mu0;            // incident solar zenith angle
  ...
  if (!Land.urbpoi) {
    ...
    double flx_abs_lcl[nlevsno+1][numrad_snw] = {}; //init to 0
    ...
    if ((coszen > 0.0) && (h2osno_lcl > min_snw)) {
     ...
     //for (int i = nir_bnd_bgn; i <= nir_bnd_end; ++i) { albsoi_lcl[i] = albsoi[1]; }
      ...
      //
      // 3-band weights
        // comment out 3-band - probably won't be used
      ...
      for (int bnd_idx = 0; bnd_idx < numrad_snw; ++bnd_idx) {
        ...
        for (int i = snl_top; i <= snl_btm; ++i) {
          ...
          // aerosol species 1 optical properties (within-ice BC)
          ss_alb_aer_lcl[0]        = ss_alb_bc1[bnd_idx];
          asm_prm_aer_lcl[0]       = asm_prm_bc1[bnd_idx];
          ext_cff_mss_aer_lcl[0]   = ext_cff_mss_bc1[bnd_idx];

          // aerosol species 2 optical properties
          ss_alb_aer_lcl[1]        = ss_alb_bc2[bnd_idx];
          asm_prm_aer_lcl[1]       = asm_prm_bc2[bnd_idx];
          ext_cff_mss_aer_lcl[1]   = ext_cff_mss_bc2[bnd_idx];
#endif
          //mgf--
        ...
        } // end Weighted Mie parameters of each layer
        
        // DELTA transformations, if requested
        if (DELTA == 1) {
          for (int i = snl_top; i <= snl_btm; ++i) {
            g_star[i]     = g[i] / (1.0 + g[i]);
            omega_star[i] = ((1.0 - pow(g[i], 2.0)) * omega[i]) / (1.0 - (omega[i] * pow(g[i], 2.0)));
            tau_star[i]   = (1.0 - (omega[i] * pow(g[i], 2.0))) * tau[i];
          }
        } else {
          for (int i = snl_top; i <= snl_btm; ++i) {
            g_star[i]     = g[i];
        ...
        for (int i = snl_top; i <= snl_btm; ++i) {
          ...
          if (trntdr[i] > trmin ) {
            ...
            // u(w,gg,e)        = c1p5*(c1 - w*gg)/e
            // el(w,gg)         = sqrt(c3*(c1-w)*(c1 - w*gg))
            lm = std::sqrt(c3 * (c1-ws) * (c1 - ws*gs)); // lm = el(ws,gs)
            ue = c1p5 * (c1 - ws*gs) / lm; //ue = u(ws,gs,lm)
            extins = std::max(exp_min, exp(-lm*ts));
      ...
      if ((mu_not < mu_75) && (flg_slr_in == 1)) {
        sza_c1 = sza_a0 + sza_a1 * mu_not + sza_a2 * pow(mu_not, 2.0);
        sza_c0 = sza_b0 + sza_b1 * mu_not + sza_b2 * pow(mu_not, 2.0);
        sza_factor = sza_c1 * (log10(snw_rds_lcl[snl_top] * c1) - c6) + sza_c0;
        flx_sza_adjust  = albout[1] * (sza_factor-c1) * flx_wgt_sum;
        albout[1] *= sza_factor;
        flx_abs[snl_top][1] = flx_abs[snl_top][1] - flx_sza_adjust;
      }
    ...
    // There is either zero snow, or no sun
             ...
             albout[0] = 0.0;
             albout[1] = 0.0;
    } // if column has snow and coszen > 0
  } // end if (!Land.urbpoi)
} // end SNICAR_AD_RT
