...
double ns::declination_angle(int doy) {
  return 23.45 * ELM_PI / 180.0 * cos(TWO_PI / 365.0 * (172.0 - doy));
}

// declination angle calc from ELM lnd_import szenith()/shr_orb_cosz()
double ns::declination_angle2(int doy) {
  return 23.45 * ELM_PI / 180.0 * sin(TWO_PI * (284.0 + doy) / 365.0);
}
...
double ns::dt_radians (double dt) {
...
double ns::dt_end_rad (double t_start, double dtrad) {
...
double ns::integrate_cosz(double t_start, double t_end, double dtrad, double cos_h, 
  ...
  return (ha[1] > ha[0] || ha[3] > ha[2]) ? (aa * (ha[1] - ha[0]) + bb * (sin(ha[1]) - sin(ha[0]))) / dtrad +
        (aa * (ha[3] - ha[2]) + bb * (sin(ha[3]) - sin(ha[2]))) / dtrad : 0.0;
}
