...
double ns::prev_tgrnd(const int& snl, const double& frac_sno_eff, const double& frac_h2osfc, const double& t_h2osfc_bef,
...
double ns::delta_t(const double& t_grnd, const double& t_grnd0) { return t_grnd - t_grnd0; }
...
double ns::evap_ratio(const double& h2osoi_ice_snotop, const double& h2osoi_liq_snotop, const double& dtime,
...
// tssbef_snotop = tssbef(ELM::nlevsno-snl)
// tssbef_soitop = tssbef(EML::nlevsno)
void ns::initial_flux_calc(const bool& urbpoi, const int& snl, const double& frac_sno_eff, const double& frac_h2osfc,
...
// tssbef_snotop = tssbef(ELM::nlevsno-snl)
// tssbef_soitop = tssbef(EML::nlevsno)
void ns::update_surface_fluxes(const bool& urbpoi, const bool& do_capsnow, const int& snl, const double& dtime,
  ...
  if (!urbpoi) {
    ...
    eflx_soil_grnd = ((1.0 - frac_sno_eff) * sabg_soil + frac_sno_eff * sabg_snow) + dlrad +
                     (1.0 - frac_veg_nosno) * emg * forc_lwrad - emg * ELM_STEBOL * lw_grnd -
                     pow(emg * ELM_STEBOL * t_grnd0, 3.0) * (4.0 * tinc) - (eflx_sh_grnd + qflx_evap_soi * htvp);
  } else {
    ...
    // eflx_lwrad_del = 4.0 * emg * ELM::ELM_STEBOL * pow(t_grnd0, 3.0) * tinc;
...
// tssbef_snotop = tssbef(ELM::nlevsno-snl)
// tssbef_soitop = tssbef(EML::nlevsno)
void ns::lwrad_outgoing(const bool& urbpoi, const int& snl, const int& frac_veg_nosno, const double& forc_lwrad,
  ...
  if (!urbpoi) {
    ...
    eflx_lwrad_out = ulrad + (1 - frac_veg_nosno) * (1.0 - emg) * forc_lwrad +
                     (1 - frac_veg_nosno) * emg * ELM_STEBOL * lw_grnd +
                     4.0 * emg * ELM_STEBOL * pow(t_grnd0, 3.0) * tinc;
