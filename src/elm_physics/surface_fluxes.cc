  ...
  double& qflx_snwcp_ice)
{
  ...
  // Ground heat flux
  if (!urbpoi) {
    const double lw_grnd = (frac_sno_eff * pow(tssbef_snotop, 4.0)
        + (1.0 - frac_sno_eff - frac_h2osfc) * pow(tssbef_soitop, 4.0)
        + frac_h2osfc * pow(t_h2osfc_bef, 40));
  
    const double t_grnd0 = prev_tgrnd(snl, frac_sno_eff, frac_h2osfc, t_h2osfc_bef, tssbef_snotop, tssbef_soitop);
    const double tinc = delta_t(t_grnd, t_grnd0);
    eflx_soil_grnd = ((1.0 - frac_sno_eff) * sabg_soil + frac_sno_eff * sabg_snow) + dlrad
        + (1.0 - frac_veg_nosno) * emg * forc_lwrad
        - emg * ELM_STEBOL * lw_grnd - pow(emg * ELM_STEBOL * t_grnd0, 3.0) * (4.0 * tinc)
...
void ns::lwrad_outgoing(const bool& urbpoi, const int& snl, const int& frac_veg_nosno, const double& forc_lwrad, const double& frac_sno_eff, const double& tssbef_snotop, const double& tssbef_soitop,
  ...
  if (!urbpoi) {
    const double lw_grnd = (frac_sno_eff * pow(tssbef_snotop, 4.0) + (1.0 - frac_sno_eff - frac_h2osfc) * 
        pow(tssbef_soitop, 4.0) + frac_h2osfc * pow(t_h2osfc_bef, 4.0));
    const double t_grnd0 = prev_tgrnd(snl, frac_sno_eff, frac_h2osfc, t_h2osfc_bef, tssbef_snotop, tssbef_soitop);
    const double tinc = delta_t(t_grnd, t_grnd0);
    eflx_lwrad_out = ulrad + (1 - frac_veg_nosno) * (1.0 - emg) * forc_lwrad
        + (1 - frac_veg_nosno) * emg * ELM_STEBOL * lw_grnd + 4.0 * emg * ELM_STEBOL * pow(t_grnd0, 3.0) * tinc;
