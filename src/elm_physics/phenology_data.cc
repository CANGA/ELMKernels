...
ELM::PhenologyDataManager::PhenologyDataManager(const size_t ncells, const size_t npfts)
    : mlai_("mlai", 3, ncells), msai_("msai", 3, ncells), mhtop_("mhtop", 3, ncells), mhbot_("mhbot", 3, ncells),
      ncells_(ncells), npfts_(npfts), initialized_(false),
      need_new_data_(false), phenology_names_{{{"MONTHLY_LAI", mlai_},
                                               {"MONTHLY_SAI", msai_},
                                               {"MONTHLY_HEIGHT_TOP", mhtop_},
                                               {"MONTHLY_HEIGHT_BOT", mhbot_}}} {}

// read data from file
// either all three months of data, or new single month
void ELM::PhenologyDataManager::read_data(const Utils::DomainDecomposition<2>& dd, const std::string& filename,
...
void ELM::PhenologyDataManager::read_month(const Utils::DomainDecomposition<2>& dd, const std::string& filename,
                                           const std::string& varname, const size_t month, const ArrayI1& vtype,
                                           const int arr_idx, ArrayD2& arr) {
