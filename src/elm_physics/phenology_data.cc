...
ELM::PhenologyDataManager::PhenologyDataManager(const size_t ncells, const size_t npfts) 
  : mlai_("mlai", 3, ncells),
    msai_("msai", 3, ncells),
...
void ELM::PhenologyDataManager::read_data(const Utils::DomainDecomposition<2> &dd, const std::string& filename, const Utils::Date& model_time,
  const ArrayI1& vtype) {
  if (!initialized_) {
...
void ELM::PhenologyDataManager::get_data(const Utils::Date& model_time, const ArrayD1& snow_depth, const ArrayD1& frac_sno, const ArrayI1& vtype,
  ...
  phenology::ComputePhenology compute_phen(mlai_, msai_, mhtop_, mhbot_, snow_depth, frac_sno,
...
void ELM::PhenologyDataManager::read_month(const Utils::DomainDecomposition<2> &dd, const std::string& filename, const std::string& varname,
...
void ELM::PhenologyDataManager::read_initial(const Utils::DomainDecomposition<2> &dd, const std::string& filename,
...
void ELM::PhenologyDataManager::read_new_month(const Utils::DomainDecomposition<2> &dd, const std::string& filename,
  const Utils::Date& model_time, const ArrayI1& vtype) {
