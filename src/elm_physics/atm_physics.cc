...
double ns::interp_forcing(const double& wt1, const double& wt2, const double& forc1, const double& forc2) {
  return forc1 * wt1 + forc2 * wt2;
}

// convert degrees K to C; bound on interval [-50,50]
double ns::tdc(const double& t) { return std::min(50.0, std::max(-50.0, (t - ELMconstants::TFRZ))); }
...
double ns::derive_forc_po2(const double& forc_pbot) { return ELMconstants::O2_MOLAR_CONST * forc_pbot; }

// derive partial CO2 pressure from atmospheric pressure
double ns::derive_forc_pco2(const double& forc_pbot) { return ELMconstants::CO2_PPMV * 1.0e-6 * forc_pbot; }
