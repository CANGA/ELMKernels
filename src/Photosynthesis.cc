  ...
  const double& dayl_factor) 
{
  ...
  // based on maximum vcmax25 But not used as defined here if using sun/shade big leaf code. Instead, will use canopy integrated scaling factors from SurfaceAlbedo.
  if (dayl_factor == 0.0) {
    kn =  0.0;
  } else {
  ...
  // Loop through canopy layers (above snow). Respiration needs to be calculated every timestep. Others are calculated only if daytime
  laican = 0.0;
  for (int iv = 0; iv < nrad; iv++) {
    ...
    // Scale for leaf nitrogen profile. If multi-layer code, use explicit profile. If sun/shade big leaf code, use canopy integrated factor.
    if (nlevcan == 1) {
      nscaler = vcmaxcint;
    } else if (nlevcan > 1) {
  ...
  for (int iv = 0; iv < nrad; iv++) {
    if (par_z[iv] <= 0.0) { // night time
      ...
      if (gs_mol[iv] < 0.0) {
        throw std::runtime_error("ELM ERROR: Negative stomatal conductance");  } //error! end run -- message??
      ...
      if (std::abs(gs_mol[iv] - gs_mol_err) > 1.0e-01) { } // write output message??
    } // night/day
  } // nrad canopy layer loop

  // Canopy photosynthesis and stomatal conductance
  // Sum canopy layer fluxes and then derive effective leaf-level fluxes (per unit leaf area), which are used in other parts of the model. Here, laican sums to either laisun or laisha.
  psncan = 0.0;
  psncan_wc = 0.0;
  psncan_wj = 0.0;
  psncan_wp = 0.0;
