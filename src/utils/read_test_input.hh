...
/*! Parse state_ and assign data to arr */
  ...
  void parseState(Array_t arr) {
    ...
    while (std::getline(state_ss, line_str)) {
      ...
      if (namefromfile == namefromarr) {
        ...
        const std::size_t len = std::distance(std::istream_iterator<std::string>(line_ss), std::istream_iterator<std::string>());
        if (len != arr.size()) {
  ...
  void compareOutput(Array_t arr, const double rel_tol=1e-15) {
    ...
    std::cout << std::boolalpha << arr.label() << " from NSTEP " << nstep_ << " passes: " << same << std::endl;
    if (!same) {
