                    ...
                    Context ctx, Runtime *runtime)
{
  ...
  //  flux_region.wait_until_valid(); // actually likely not needed, implicit wait on accessor
  ...
  runtime->destroy_index_space(ctx, gc_state_is);
}
...
void driver(int argc, char ** argv)
{
  ...
  flecsi_execute_task(top_level_task, example, single);
  flecsi_execute_task(initialize_phenology_task, example, single);
  flecsi_execute_task(initialize_forcing_task, example, single);
