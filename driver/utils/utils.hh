...
template <size_t D> struct DomainDecomposition {
...
#ifdef HAVE_MPI
  MPI_Comm comm;
  DomainDecomposition() : comm(MPI_COMM_WORLD) {}
#else
  int comm;
  DomainDecomposition() : comm(-1) {}
...
DomainDecomposition<1> create_domain_decomposition_1D(int nprocs, GO n_global, int proc_index);

DomainDecomposition<2> create_domain_decomposition_2D(std::array<int, 2> n_procs, std::array<GO, 2> n_global,
...
inline time_point_type time() { return std::chrono::high_resolution_clock::now(); }
