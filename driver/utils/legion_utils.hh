...
namespace Impl {
// projection
template <size_t DIM> class LocalProjectionFunction : public ProjectionFunctor {
public:
  LocalProjectionFunction(const Point<DIM> &partition, Runtime *rt) : ProjectionFunctor(rt), partition_(partition) {}
  virtual ~LocalProjectionFunction() {}

public:
  virtual LogicalRegion project(const Mappable *mappable, unsigned index, LogicalRegion upper_bound,
                                const DomainPoint &point) override {
    assert(false);
  }
  virtual LogicalRegion project(const Mappable *mappable, unsigned index, LogicalPartition upper_bound,
                                const DomainPoint &point) override {
...
template <size_t DIM> struct Data {
  Data(const std::string &name_, Context ctx_, Runtime *runtime_) : name(name_), ctx(ctx_), runtime(runtime_) {
    field_space = runtime->create_field_space(ctx);
    allocator_ = runtime->create_field_allocator(ctx, field_space);
  }

  Data(const std::string &name_, Context ctx_, Runtime *runtime_, const Point<DIM> shape_,
  ...
  Data(const std::string &name_, Context ctx_, Runtime *runtime_, const Point<DIM> shape_,
       const Point<DIM> blocked_shape_, const std::vector<std::string> &field_names_)
  ...
  void initShape(const Point<DIM> shape_, const Point<DIM> blocked_shape_) {
    ...
    Point<DIM> block;
    for (int i = 0; i != DIM; ++i)
      block[i] = shape[i] / blocked_shape[i];
  ...
  template <typename T> FieldID allocateField_() { return allocator_.allocate_field(sizeof(T), AUTO_GENERATE_ID); }

  template <typename T> void addField(const std::string &fieldname) {
    FieldID fid = allocateField_<T>();
    field_ids[fieldname] = fid;
    field_names.push_back(fieldname);
  }

  template <typename T> void addFields(const std::vector<std::string> &fieldnames) {
    for (auto fname : fieldnames) {
      addField<T>(fname);
    }
  }

  template <typename... Ts> void addVariadicFields(const std::vector<std::string> &fieldnames) {
...
void read_and_reshape_phenology(const std::string &dir, const std::string &basename, const std::string &phenology_type,
                                int start_year, int start_month, int n_months, int n_lat_local, int n_lon_local,
                                int n_pfts, Array_t &arr) {
  ELM::Utils::Array<double, 4> arr_for_read(n_months, n_pfts, n_lat_local, n_lon_local);
  read_phenology(dir, basename, phenology_type, start_year, start_month, arr_for_read);
...
template <class Array_t>
void read_and_reshape_forcing(const std::string &dir, const std::string &basename, const std::string &forcing_type,
                              int start_year, int start_month, int n_months, int n_timesteps, int n_lat_local,
                              int n_lon_local, Array_t &arr) {
...
template <class Array_t>
void convert_precip_to_rain_snow(Array_t &rain, Array_t &snow, const Array_t &temp, int nt, int ng) {
  for (int k = 0; k < nt; k++) {
    for (int j = 0; j < ng; j++) {
