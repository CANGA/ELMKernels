...
void ELM::kokkos_albedo_snicar(const std::shared_ptr<ELM::ELMState<ViewI1, ViewI2, ViewD1, ViewD2, ViewD3, ViewPSN1>>& S,
                               ...
                               const std::shared_ptr<ELM::PFTData<ViewD1, ViewD2>>& pft_data)
{
  ...
  invoke_kernel(albedo_kernels, std::make_tuple(S->h2osno.extent(0)), "kokkos_albedo_and_snicar");
}
