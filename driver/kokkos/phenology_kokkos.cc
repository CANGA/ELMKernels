...
void ELM::update_phenology(ELMStateType& S,
  ...
  const std::string& fname_surfdata)
{
  ...
  bool phen_updated{false};
  auto phen_data = S.phen_data.get();
  if (phen_data->need_data()) {

    // temporarily copy device data into host
    auto host_phen_views = ELM::get_phen_host_views(*phen_data);
    ...
    auto host_vtype = NS::create_mirror_view(S.vtype);
    phen_updated = phen_data->read_data(host_phen_views, fname_surfdata, current, host_vtype); // if needed
